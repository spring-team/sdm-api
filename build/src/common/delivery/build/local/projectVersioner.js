"use strict";
/*
 * Copyright © 2018 Atomist, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const automation_client_1 = require("@atomist/automation-client");
const MessageClient_1 = require("@atomist/automation-client/spi/message/MessageClient");
const _ = require("lodash");
const sdmVersionIngester_1 = require("../../../../ingesters/sdmVersionIngester");
/**
 * Version the project with a build specific version number
 * @param projectLoader used to load projects
 */
function executeVersioner(projectLoader, projectVersioner) {
    return (rwlc) => __awaiter(this, void 0, void 0, function* () {
        const { status, credentials, id, context, progressLog } = rwlc;
        return projectLoader.doWithProject({ credentials, id, context, readOnly: false }, (p) => __awaiter(this, void 0, void 0, function* () {
            const version = yield projectVersioner(status, p, progressLog);
            const sdmVersion = {
                sha: status.commit.sha,
                branch: id.branch,
                version,
                repo: {
                    owner: status.commit.repo.owner,
                    name: status.commit.repo.name,
                    providerId: status.commit.repo.org.provider.providerId,
                },
            };
            yield context.messageClient.send(sdmVersion, MessageClient_1.addressEvent(sdmVersionIngester_1.SdmVersionRootType));
            return automation_client_1.Success;
        }));
    });
}
exports.executeVersioner = executeVersioner;
function readSdmVersion(owner, name, providerId, sha, branch, context) {
    return __awaiter(this, void 0, void 0, function* () {
        const version = yield context.graphClient.query({
            name: "SdmVersionForCommit",
            variables: {
                name: [name],
                owner: [owner],
                providerId: [providerId],
                sha: [sha],
                branch: [branch],
            },
        });
        return _.get(version, "SdmVersion[0].version");
    });
}
exports.readSdmVersion = readSdmVersion;
//# sourceMappingURL=projectVersioner.js.map