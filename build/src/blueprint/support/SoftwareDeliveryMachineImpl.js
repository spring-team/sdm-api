 ...
 */
class SoftwareDeliveryMachineImpl extends ListenerRegistrationSupport_1.ListenerRegistrationSupport {
    /**
    ...
    constructor(name, opts) {
        ...
        this.artifactFinder = () => new FindArtifactOnImageLinked_1.FindArtifactOnImageLinked(wellKnownGoals_1.ArtifactGoal, this.opts.artifactStore, this.artifactListenerRegistrations, this.opts.projectLoader, this.opts.credentialsResolver);
    ...
    get onRepoCreation() {
        return this.repoCreationListeners.length > 0 ?
            () => new OnRepoCreation_1.OnRepoCreation(this.repoCreationListeners, this.opts.credentialsResolver) :
            undefined;
    }
    get onNewRepoWithCode() {
        return this.newRepoWithCodeActions.length > 0 ?
            () => new OnFirstPushToRepo_1.OnFirstPushToRepo(this.newRepoWithCodeActions, this.opts.credentialsResolver) :
            undefined;
    }
    get semanticDiffReactor() {
        return this.fingerprintDifferenceListeners.length > 0 ?
            () => new ReactToSemanticDiffsOnPushImpact_1.ReactToSemanticDiffsOnPushImpact(this.fingerprintDifferenceListeners, this.opts.credentialsResolver) :
            undefined;
    }
    get goalSetting() {
        ...
        return {
            eventHandlers: [() => new SetGoalsOnPush_1.SetGoalsOnPush(this.opts.projectLoader, this.goalSetters, this.goalsSetListeners, this.goalFulfillmentMapper, this.opts.credentialsResolver)],
    ...
    get goalConsequences() {
        return {
            eventHandlers: [
                ...
                () => new RespondOnGoalCompletion_1.RespondOnGoalCompletion(this.opts.credentialsResolver, this.goalCompletionListeners)
            ],
            commandHandlers: [],
        };
    }
    get notifyOnDeploy() {
        return this.deploymentListeners.length > 0 ?
            () => new OnDeployStatus_1.OnDeployStatus(this.deploymentListeners, this.opts.credentialsResolver) :
            undefined;
    }
    get onVerifiedStatus() {
        return this.verifiedDeploymentListeners.length > 0 ?
            () => new OnVerifiedDeploymentStatus_1.OnVerifiedDeploymentStatus(this.verifiedDeploymentListeners, this.opts.credentialsResolver) :
            undefined;
    }
    get disposal() {
        return {
    ...
    get eventHandlers() {
        return this.supportingEvents
            .concat(() => new FulfillGoalOnRequested_1.FulfillGoalOnRequested(this.goalFulfillmentMapper, this.opts.projectLoader, this.opts.logFactory))
            .concat(_.flatten(this.allFunctionalUnits.map(fu => fu.eventHandlers)))
            .concat([
            this.userJoiningChannelListeners.length > 0 ?
                () => new OnUserJoiningChannel_1.OnUserJoiningChannel(this.userJoiningChannelListeners, this.opts.credentialsResolver) :
                undefined,
            this.buildListeners.length > 0 ?
                () => new InvokeListenersOnBuildComplete_1.InvokeListenersOnBuildComplete(this.buildListeners, this.opts.credentialsResolver) :
                undefined,
            this.tagListeners.length > 0 ? () => new OnTag_1.OnTag(this.tagListeners, this.opts.credentialsResolver) : undefined,
            this.newIssueListeners.length > 0 ? () => new NewIssueHandler_1.NewIssueHandler(this.newIssueListeners, this.opts.credentialsResolver) : undefined,
            this.updatedIssueListeners.length > 0 ?
                () => new UpdatedIssueHandler_1.UpdatedIssueHandler(this.updatedIssueListeners, this.opts.credentialsResolver) :
                undefined,
            this.closedIssueListeners.length > 0 ?
                () => new ClosedIssueHandler_1.ClosedIssueHandler(this.closedIssueListeners, this.opts.credentialsResolver) :
                undefined,
            this.channelLinkListeners.length > 0 ?
                () => new OnChannelLink_1.OnChannelLink(this.opts.projectLoader, this.channelLinkListeners, this.opts.credentialsResolver) :
                undefined,
            this.pullRequestListeners.length > 0 ?
                () => new OnPullRequest_1.OnPullRequest(this.opts.projectLoader, this.pullRequestListeners, this.opts.credentialsResolver) : undefined,
            this.repoOnboardingListeners.length > 0 ?
                () => new OnRepoOnboarded_1.OnRepoOnboarded(this.repoOnboardingListeners, this.opts.credentialsResolver) :
                undefined,
            this.onRepoCreation,
    ...
    addBuildRules(...rules) {
        ...
        _.flatten(rules).forEach(r => this.addGoalImplementation(r.name, wellKnownGoals_1.BuildGoal, executeBuild_1.executeBuild(this.opts.projectLoader, r.value), {
            pushTest: r.pushTest,
            logInterpreter: r.value.logInterpreter,
        })
            .addGoalImplementation(r.name, wellKnownGoals_1.JustBuildGoal, executeBuild_1.executeBuild(this.opts.projectLoader, r.value), {
            pushTest: r.pushTest,
            logInterpreter: r.value.logInterpreter,
        }));
        return this;
    ...
    addDeployRules(...rules) {
        ...
        _.flatten(rules).forEach(r => {
            // deploy
            this.addGoalImplementation(r.name, r.value.deployGoal, executeDeploy_1.executeDeploy(this.opts.artifactStore, r.value.endpointGoal, r.value), {
                pushTest: r.pushTest,
...
exports.SoftwareDeliveryMachineImpl = SoftwareDeliveryMachineImpl;
//# sourceMappingURL=SoftwareDeliveryMachineImpl.js.map