...
const mavenLogInterpreter_1 = require("../../../../../../../src/common/delivery/build/local/maven/mavenLogInterpreter");
describe("MavenLogInterpreter", () => {
    it("should handle empty log", () => {
        const r = mavenLogInterpreter_1.MavenLogInterpreter("");
        assert(!!r);
        assert.equal(r.message, "Failed with empty log");
        assert(!r.data.timeMillis);
    });
    it("should handle short failure log", () => {
        const r = mavenLogInterpreter_1.MavenLogInterpreter(Fail1);
        assert(!!r);
    ...
    it("should handle longer failure log", () => {
        const r = mavenLogInterpreter_1.MavenLogInterpreter(Fail2);
        assert(!!r);
        assert(r.relevantPart.length > 30);
    ...
    it("should handle failure with tests", () => {
        const r = mavenLogInterpreter_1.MavenLogInterpreter(FailWithTests);
        assert(!!r);
        assert(r.relevantPart.length > 30);
    ...
    it("should handle success log", () => {
        const r = mavenLogInterpreter_1.MavenLogInterpreter(Success1);
        assert(!!r);
...
[ERROR] For more information about the errors and possible solutions, please read the following articles:
[ERROR] [Help 1] http://cwiki.apache.org/confluence/display/MAVEN/MojoFailureException`;
//# sourceMappingURL=mavenLogInterpreterTest.js.map