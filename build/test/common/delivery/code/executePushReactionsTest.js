...
const executePushReactions_1 = require("../../../../src/common/delivery/code/executePushReactions");
...
function react(invocations, stopTheWorld) {
    return {
        name: "hatred",
        ...
        action: (cri) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (stopTheWorld) {
                return PushReactionRegistration_1.PushReactionResponse.failGoals;
            }
        }),
    };
}
describe("executePushReactions", () => {
    it("stops the world", () => __awaiter(this, void 0, void 0, function* () {
        const id = new GitHubRepoRef_1.GitHubRepoRef("a", "b");
        const p = InMemoryProject_1.InMemoryProject.from(id);
        const invocations = [];
        const ge = executePushReactions_1.executePushReactions(new SingleProjectLoader_1.SingleProjectLoader(p), [react(invocations, true)]);
        const r = yield ge(fakeRunWithLogContext_1.fakeRunWithLogContext(id));
        assert.equal(invocations.length, 1);
        assert(!r.requireApproval);
        assert.equal(r.code, 1);
    }));
    it("is invoked but doesn't stop the world", () => __awaiter(this, void 0, void 0, function* () {
        const id = new GitHubRepoRef_1.GitHubRepoRef("a", "b");
        const p = InMemoryProject_1.InMemoryProject.from(id);
        const invocations = [];
        const ge = executePushReactions_1.executePushReactions(new SingleProjectLoader_1.SingleProjectLoader(p), [react(invocations, false)]);
        const r = yield ge(fakeRunWithLogContext_1.fakeRunWithLogContext(id));
        assert.equal(invocations.length, 1);
...
//# sourceMappingURL=executePushReactionsTest.js.map