...
const RequestDownstreamGoalsOnGoalSuccess_1 = require("../../src/handlers/events/delivery/goals/RequestDownstreamGoalsOnGoalSuccess");
const sdmGoalIngester_1 = require("../../src/ingesters/sdmGoalIngester");
describe("Putting SdmGoal events together", () => {
    it("Lets the event we just received override out-of-date query results", () => {
        const successfulSdmGoal = successEvent.data.SdmGoal[0];
        const result = RequestDownstreamGoalsOnGoalSuccess_1.sumSdmGoalEventsByOverride(queryResult.SdmGoal, [successfulSdmGoal]);
        assert.equal(result.length, queryResult.SdmGoal.length);
        const statusOfInterest = result.filter(sg => sdmGoalIngester_1.goalKeyEquals(sg, successfulSdmGoal));
...
//# sourceMappingURL=sdmGoalSummationTest.js.map