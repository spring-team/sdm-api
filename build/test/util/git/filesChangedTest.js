...
const filesChangedSince_1 = require("../../../src/util/git/filesChangedSince");
describe("filesChanged", () => {
    ...
    describe("anyFileChangedSuchThat", () => {
        it("should recognize passing", () => {
            assert(filesChangedSince_1.anyFileChangedSuchThat(["path", "path/2"], path => path.startsWith("path")));
        });
        it("should recognize test", () => {
            assert(!filesChangedSince_1.anyFileChangedSuchThat(["path", "path/2"], path => path.startsWith("xpath")));
        });
    });
    describe("anyFileChangedWithExtension", () => {
        it("should recognize extension", () => {
            assert(filesChangedSince_1.anyFileChangedWithExtension(["path.c", "path/two.java"], ["c"]));
        });
        it("should recognize extension from list", () => {
            assert(filesChangedSince_1.anyFileChangedWithExtension(["path.c", "path/two.java"], ["java"]));
        });
        it("should recognize extension without dot", () => {
            assert(filesChangedSince_1.anyFileChangedWithExtension(["path.c", "path/two.java"], ["c"]));
        });
        it("should recognize no suc extension", () => {
            assert(!filesChangedSince_1.anyFileChangedWithExtension(["path.c", "path/two.java"], ["graphql"]));
        });
    });
});
/* tslint:disable */
...
`;
*/ 
//# sourceMappingURL=filesChangedTest.js.map